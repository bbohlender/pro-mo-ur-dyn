...
export class Queue {
     ...
     */
    constructor(private compare: (v1: unknown, v2: unknown, currTr?: unknown) => number) {}
     ...
     */
    push(entry: QueueEntry, currTrans?: NestedTransformation): void {
        if (entry.stack.length === 0) {
            this.results.push(entry.value)
            return
        }
        let i = this.list.length - 1
        while (i > 0 && this.compare(entry.value.raw, this.list[i].value.raw, currTrans) < 0) {
            i--
        }
