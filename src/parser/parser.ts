...
const grammar: Grammar = {
    ...
    ParserRules: [
        ...
        {
            name: "DescriptionsDefinition$ebnf$1",
            symbols: ["DescriptionsDefinition$ebnf$1", "DescriptionDefinition"],
            postprocess: (d) => d[0].concat([d[1]]),
        },
        {
            name: "DescriptionsDefinition",
            symbols: ["ws", "DescriptionsDefinition$ebnf$1"],
            postprocess: ([, descriptions]) =>
                descriptions.reduce((prev: any, [identifier, description]: [string, any]) => {
                    prev[identifier] = description
                    return prev
                }, {}),
        },
        {
            name: "DescriptionDefinition$ebnf$1$subexpression$1",
            symbols: [
        ...
        {
            name: "DescriptionDefinition$ebnf$1",
            symbols: ["DescriptionDefinition$ebnf$1$subexpression$1"],
            postprocess: id,
        },
        { name: "DescriptionDefinition$ebnf$1", symbols: [], postprocess: () => null },
        {
            ...
            postprocess: ([{ value: identifier }, , initialVariables, , , nouns]) => [
                identifier,
                { initialVariables: initialVariables?.[2] ?? {}, nouns, rootNounIdentifier: Object.keys(nouns)[0] },
            ],
        },
        { name: "InitialVariables$ebnf$1", symbols: [] },
        {
            name: "InitialVariables$ebnf$1",
            symbols: ["InitialVariables$ebnf$1", "InitialVariable"],
            postprocess: (d) => d[0].concat([d[1]]),
        },
        {
            name: "InitialVariables",
            symbols: ["InitialVariables$ebnf$1"],
            postprocess: ([initialVariables]) =>
                initialVariables.reduce((prev: any, [identifier, value]: [string, any]) => {
        ...
        },
        {
            ...
            postprocess: ([{ value: identifier }, , , , value]) => [identifier, value],
        },
        { name: "NounDefinitions$ebnf$1", symbols: [] },
        {
            name: "NounDefinitions$ebnf$1",
            symbols: ["NounDefinitions$ebnf$1", "NounDefinition"],
            postprocess: (d) => d[0].concat([d[1]]),
        },
        {
            name: "NounDefinitions",
            symbols: ["NounDefinitions$ebnf$1"],
            postprocess: ([nouns]) =>
                nouns.reduce((prev: any, [identifier, transformation]: [string, any]) => {
                    prev[identifier] = transformation
                    return prev
                }, {}),
        },
        {
            ...
            postprocess: ([{ value: identifier }, , , , transformation]) => [identifier, transformation],
        },
        {
            name: "Transformation",
            symbols: ["ParallelTransformations"],
            postprocess: ([transformation]) => transformation,
        },
        { name: "ParallelTransformations$ebnf$1", symbols: ["ParallelTransformation"] },
        {
            name: "ParallelTransformations$ebnf$1",
            symbols: ["ParallelTransformations$ebnf$1", "ParallelTransformation"],
            postprocess: (d) => d[0].concat([d[1]]),
        },
        {
            name: "ParallelTransformations",
            symbols: ["ParallelTransformations$ebnf$1", "SequentialTransformations"],
            postprocess: ([transformations, transformation]) => ({
                type: "parallel",
                children: [...transformations, transformation],
            }),
        },
        {
            name: "ParallelTransformations",
            symbols: ["SequentialTransformations"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "ParallelTransformation",
            symbols: ["SequentialTransformations", "ws", lexer.has("parallel") ? { type: "parallel" } : parallel, "ws"],
            postprocess: ([transformation]) => transformation,
        },
        { name: "SequentialTransformations$ebnf$1", symbols: ["SequentialTransformation"] },
        {
            name: "SequentialTransformations$ebnf$1",
            symbols: ["SequentialTransformations$ebnf$1", "SequentialTransformation"],
            postprocess: (d) => d[0].concat([d[1]]),
        },
        {
            name: "SequentialTransformations",
            symbols: ["SequentialTransformations$ebnf$1", "OrOperation"],
            postprocess: ([transformations, transformation]) => ({
                type: "sequential",
                children: [...transformations, transformation],
            }),
        ...
        },
        {
            ...
            postprocess: ([transformation1, , , , transformation2]) => ({
                type: "||",
                children: [transformation1, transformation2],
            }),
        },
        ...
        { name: "AndOperation", symbols: ["NegateOperation"], postprocess: ([transformation]) => transformation },
        {
            ...
            postprocess: ([, , transformation]) => ({ type: "!", children: [transformation] }),
        },
        {
            name: "NegateOperation",
            symbols: ["ComparisonOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "ComparisonOperation",
            symbols: ["EquityOperation"],
            postprocess: ([transformation]) => transformation,
        },
        ...
        {
            name: "EquityOperation",
            symbols: ["RelationalOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "EqualOperation",
        ...
        },
        {
            ...
            symbols: ["SmallerOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "RelationalOperation",
            symbols: ["SmallerEqualOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "RelationalOperation",
            symbols: ["GreaterOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "RelationalOperation",
            symbols: ["GreaterEqualOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "RelationalOperation",
            symbols: ["ArithmeticOperation"],
            postprocess: ([transformation]) => transformation,
        },
        {
        ...
        { name: "Random$ebnf$1", symbols: ["Random$ebnf$1", "RandomStep"], postprocess: (d) => d[0].concat([d[1]]) },
        {
            ...
            postprocess: ([, cases]) => ({
                ...
                probabilities: cases.map(({ probability }: any) => probability),
                children: cases.map(({ transformation }: any) => transformation),
            }),
        },
        {
            name: "RandomStep",
            ...
            postprocess: ([, { value }, , , , , transformation]) => ({
                probability: Number.parseFloat(value) / 100,
                transformation,
            }),
        },
        {
            ...
            postprocess: ([{ value }, , transformations]) => ({
                type: "operation",
        ...
        },
        {
            ...
            symbols: ["ws", "Parameters"],
            postprocess: ([, transformations]) => transformations,
        },
        ...
        {
            name: "Parameters",
            symbols: ["Parameters$ebnf$1", "Transformation"],
            postprocess: ([transformations, transformation]) => [...transformations, transformation],
        },
        {
            name: "Parameter",
            symbols: ["Transformation", "ws", lexer.has("comma") ? { type: "comma" } : comma, "ws"],
            postprocess: ([transformation]) => transformation,
        },
        {
            name: "NounReference",
            symbols: [lexer.has("identifier") ? { type: "identifier" } : identifier],
            postprocess: ([{ value }]) => ({ type: "nounReference", nounIdentifier: value }),
        },
        ...
        },
        {
            ...
            postprocess: ([, , { value: identifier }, , , , transformation]) => ({
                type: "setVariable",
                identifier,
        ...
        { name: "Conditional", symbols: ["Switch"], postprocess: ([value]) => value },
        {
            ...
            postprocess: ([, , conditionTransformationId, , ifTransformationId, , elseTransformationId]) => ({
                type: "if",
                children: [conditionTransformationId, ifTransformationId, elseTransformationId],
            }),
        },
        {
            name: "Then",
        ...
        { name: "Switch$ebnf$1", symbols: ["Switch$ebnf$1", "SwitchCases"], postprocess: (d) => d[0].concat([d[1]]) },
        {
            ...
            postprocess: ([, , transformation, , , cases]) => ({
                type: "switch",
                cases: cases.map(({ caseValues }: any) => caseValues),
                children: [transformation, ...cases.map(({ transformation }: any) => transformation)],
            }),
        },
        ...
        {
            name: "SwitchCases",
            symbols: ["ws", "SwitchCases$ebnf$1", "Transformation"],
            postprocess: ([, caseValues, transformation]) => ({ caseValues, transformation }),
        },
        {
            name: "SwitchCase",
            symbols: [
