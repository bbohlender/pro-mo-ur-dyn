...
export function DeriveHeader() {
    ...
    const thresholdBuildings = useStore((state) => state.deriveThresholdBuildings)
    return (
        <Panel className="rounded gap-3 p-3 flex flex-col">
            ...
            <div className="form-control flex-row items-center mr-5">
                <label className="label mr-3">
                    <span className="label-text">Threshold Buildings </span>
                </label>
                <input
                    type="range"
                    min="0"
                    max="1"
                    step="0.01"
                    value={thresholdBuildings}
                    onChange={(e) => useStore.getState().setDeriveThresholdBuildings(e.target.valueAsNumber)}
                    className="range range-xs"
                />
            </div>
